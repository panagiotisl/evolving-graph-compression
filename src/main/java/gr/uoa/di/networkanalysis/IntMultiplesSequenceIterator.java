...
public class IntMultiplesSequenceIterator implements LazyIntIterator {
    ...
    public IntMultiplesSequenceIterator(final int left[], final int len[]) {
        this(left, len, left != null ? left.length : 0);
    }
    ...
    public IntMultiplesSequenceIterator(final int left[], final int len[], final int n) {
        this.left = left;
        this.len = len;
        this.remaining = n;
    ...
    public int nextInt() {
        ...
        currIndex++;
        if (currIndex == len[currMultiples]) advance();
        return next;
    }
    ...
    public int skip(final int n) {
        ...
        while(skipped < n && remaining != 0) {
            if (n - skipped < len[currMultiples] - currIndex) {
                currIndex += (n - skipped);
                return n;
            }
            else {
                skipped += len[currMultiples] - currIndex;
                advance();
            }
        }
