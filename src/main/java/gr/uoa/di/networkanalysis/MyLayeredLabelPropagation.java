 ...
 * <em>symmetric</em> graph provided with the {@linkplain #MyLayeredLabelPropagation(ImmutableGraph, int[], long, boolean) constructor}
...
public class MyLayeredLabelPropagation {

	private final static Logger LOGGER = LoggerFactory.getLogger(MyLayeredLabelPropagation.class);
	 ...
	 */
	public MyLayeredLabelPropagation(final ImmutableGraph symGraph, final long seed) throws IOException {
	 ...
	 * @param seed a random seed.
	 */
	public MyLayeredLabelPropagation(final ImmutableGraph symGraph, final int[] startPerm, final long seed) throws IOException {
	 ...
	 */
	public MyLayeredLabelPropagation(final ImmutableGraph symGraph, final int[] startPerm, final long seed, final boolean exact) throws IOException {
	 ...
	 * @param exact a boolean flag that forces the algorithm to run exactly.
	 */
	public MyLayeredLabelPropagation(final ImmutableGraph symGraph, final int[] startPerm, final int numberOfThreads, final long seed, final boolean exact) throws IOException {
	...
	private final class GapCostThread extends Thread {
		...
		public void run() {
			...
			final int numNodes = MyLayeredLabelPropagation.this.n;
			final long numArcs = MyLayeredLabelPropagation.this.symGraph.numArcs();
			...
			for (;;) {
				...
				synchronized(MyLayeredLabelPropagation.this.cumulativeOutdegrees) {
					if (nextNode == numNodes) {
						MyLayeredLabelPropagation.this.gapCost.add(gapCost);
						break;
					}
					start = nextNode;
				...
				for (int i = start; i < end; i++) {
					...
					if (outdegree > 0) {
						...
						for (int j = outdegree; j-- != 0;) {
							try {
								int value1 = successors[j];
								int value2 = perm[value1];
								permutedSuccessors[j] = value2;
							}
							catch(ArrayIndexOutOfBoundsException e) {
								System.out.println("Caught exception for node: "+node+" j is: " + j +" outdegree is: "+outdegree);
	...
	private final class IterationThread extends Thread {
		...
		@Override
		public void run() {
			final XoRoShiRo128PlusRandom r = new XoRoShiRo128PlusRandom(MyLayeredLabelPropagation.this.seed);
			final AtomicIntegerArray label = MyLayeredLabelPropagation.this.label;
			final AtomicIntegerArray volume = MyLayeredLabelPropagation.this.volume;
			final ImmutableGraph symGraph = this.symGraph;
			final int numNodes = MyLayeredLabelPropagation.this.n;
			final long numArcs = MyLayeredLabelPropagation.this.symGraph.numArcs();
			final int[] updateList = MyLayeredLabelPropagation.this.updateList;
			final int[] startPerm = MyLayeredLabelPropagation.this.startPerm;
			final boolean[] canChange = MyLayeredLabelPropagation.this.canChange;
			final boolean exact = MyLayeredLabelPropagation.this.exact;
			final double gamma = this.gamma;
			final long granularity = Math.max(1024, numArcs >>> 9);

			int start, end;
			double delta = MyLayeredLabelPropagation.this.objectiveFunction[index];
			...
			for (;;) {
				...
				synchronized(MyLayeredLabelPropagation.this.cumulativeOutdegrees) {
					if (nextNode == numNodes) {
						MyLayeredLabelPropagation.this.objectiveFunction[index] = delta;
						break;
					}
					start = nextNode;
					final long target = nextArcs + granularity;
	...
	public static void main(final String[] args) throws IOException, JSAPException {
		final SimpleJSAP jsap = new SimpleJSAP(MyLayeredLabelPropagation.class.getName(), "Runs the Layered Label Propagation algorithm on a graph.", new Parameter[] {
		...
		final MyLayeredLabelPropagation clustering = new MyLayeredLabelPropagation(symGraph, startPerm, threads, jsapResult.userSpecified("randomSeed") ? jsapResult.getLong("randomSeed") : r.nextLong(), false);
